...
BridgeConfig readConfig(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        try {
            if (section == "vicon") {
                if (key == "input_object") config.input_object = value;
                else if (key == "output_object") config.output_object = value;
...
int main(int argc, char* argv[]) {
    ...
    if (config.input_object.empty() || config.output_object.empty()) {
        std::cerr << "Error: Input or output object name is empty.\n";
        return -1;
    }
